...
new ClassSpawn[311][classinfo];
new Iterator:ClassSpawn<311>;
PLC::Class_OnPlayerRequestClass(playerid, classid)
{
	if(pdate[playerid][_choose])return 1;
	SetInterior(playerid, 0);
	SetPlayerVirtualWorld(playerid, 0);
	new index=GetClassIndex(classid);
	SetPos(playerid,ClassSpawn[index][_x],ClassSpawn[index][_y],ClassSpawn[index][_z]);
	SetPlayerCameraPos(playerid,ClassSpawn[index][_x]+3.0,ClassSpawn[index][_y]+1.0,ClassSpawn[index][_z]+1.00);
    SetPlayerCameraLookAt(playerid,ClassSpawn[index][_x],ClassSpawn[index][_y],ClassSpawn[index][_z],2);
	SetPlayerFacingAngle(playerid, -90.0000);
	ClearAnimations(playerid);
	new randex = random(sizeof(anims));
	ApplyAnimation(playerid,anims[randex][_name],anims[randex][_action], 4.1, 0, 1, 1, 1, 1, 0);
	pdate[playerid][_skin]=ClassSpawn[index][_skinid];
	pdate[playerid][_location]=1;
	pdate[playerid][_x]=ClassSpawn[index][_x];
	pdate[playerid][_y]=ClassSpawn[index][_y];
	pdate[playerid][_z]=ClassSpawn[index][_z];
...
PLC::Class_OnPlayerRequestSpawn(playerid)
{
    if(pdate[playerid][_choose])return 1;
    pdate[playerid][_choose]=1;
	orm_update(pdate[playerid][_orm]);
	OnPlayerLogin(playerid);
    return 1;
}
PLC::Class_OnGameModeInit()
{
    new rand;
	Loop(i,2,10)
	{
		rand = random(sizeof(RandomSpawns));
		AddClass(i,RandomSpawns[rand][0],RandomSpawns[rand][1],RandomSpawns[rand][2], 0.0, 0, 0, 0, 0, 0, 0);
	}
	return 1;
}
PLC::GetClassIndex(classid)
{
    foreach(new i : ClassSpawn)
	{
	    if(ClassSpawn[i][_class]==classid)return i;
	}
	return -1;
}
PLC::AddClass(skin, Float:x, Float:y, Float:z, Float:Angle, weapon1, weapon1_ammo, weapon2, weapon2_ammo, weapon3, weapon3_ammo)
{
	if(Iter_Free(ClassSpawn) == -1) return 0;
	new r = Iter_Free(ClassSpawn);
    ClassSpawn[r][_x]=x;
    ClassSpawn[r][_y]=y;
	ClassSpawn[r][_z]=z;
	ClassSpawn[r][_Angle]=Angle;
	ClassSpawn[r][_weapon1]=weapon1;
	ClassSpawn[r][_weapon1_ammo]=weapon1_ammo;
	ClassSpawn[r][_weapon2]=weapon2;
	ClassSpawn[r][_weapon2_ammo]=weapon2_ammo;
	ClassSpawn[r][_weapon3]=weapon3;
	ClassSpawn[r][_weapon3_ammo]=weapon3_ammo;
	ClassSpawn[r][_skinid]=skin;
	ClassSpawn[r][_class]=AddPlayerClass(skin, Float:x, Float:y, Float:z, Float:Angle, weapon1, weapon1_ammo, weapon2, weapon2_ammo, weapon3, weapon3_ammo);
	Iter_Add(ClassSpawn,r);
	return 1;
}
